...
export const createConversation = async (req, res) => {
    try {
        ...
        if (!type || (type === "group" && !name) || !memberIds || !Array.isArray(memberIds) || memberIds.length === 0) {
            return res.status(400).json({ message: "Tên nhóm và các thành viên là bắt buộc" })
        }

        let conversation;

        if (type === "direct") {
            const participientId = memberIds[0];

            conversation = await Conversation.findOne({
                type: "direct",
                "participiants.userId": { $all: [userId, participientId] },
            });

            if (!conversation) {
                conversation = new Conversation({
                    type: "direct",
                    participants: [{ userId }, { userId: participientId }],
